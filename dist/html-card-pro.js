...
const MODULE_STORE_CONFIG = {
  ...
  indexUrl: "https://data.jsdelivr.com/v1/package/gh/ha-china/html-card-pro@main",
  rawBase: "https://cdn.jsdelivr.net/gh/ha-china/html-card-pro@main/mods/",
...
class HtmlTemplateCardEditor extends LitElement {
  ...
  async _loadOnlineModules(forceRefresh = false) {
    const now = Date.now();
    const timeSinceLastFetch = now - MODULE_STORE_CONFIG.lastFetchTime;
    ...
    if (timeSinceLastFetch < MODULE_STORE_CONFIG.minFetchInterval) {
      console.log(`[html-pro-card] Rate limited, wait ${Math.ceil((MODULE_STORE_CONFIG.minFetchInterval - timeSinceLastFetch) / 1000)}s`);
      if (this._storeModules.length === 0) {
    ...
    try {
      MODULE_STORE_CONFIG.lastFetchTime = now;
  ...
  async _fetchModulesFromDiscussions() {
    ...
    try {
      // Fetch file list from jsDelivr API
      const indexRes = await fetch(MODULE_STORE_CONFIG.indexUrl);
      if (!indexRes.ok) throw new Error("Failed to fetch index");

      const data = await indexRes.json();
      // jsDelivr tree API returns { files: [{ type: "directory", name: "mods", files: [...] }] }
      const modsDir = (data.files || []).find((f) => f.type === "directory" && f.name === "mods");
      const yamlFiles = (modsDir?.files || [])
      ...
      for (const file of yamlFiles) {
        try {
          ...
          const module = {
            ...
            link:
              meta.link ||
              `https://github.com/${MODULE_STORE_CONFIG.repo}/discussions/${meta.id}`,
    ...
    return html`
      <div class="store-item">
        <div class="store-item-left">
          ${image
            ? html` <img src="${image}" alt="${m.name}" loading="lazy" /> `
            : html`
...
$('#pro-desc').textContent = text;
    ...
    try {
      ...
      if (!this._rendered) {
        ...
        const hasScripts =
          ...
          Array.isArray(this._config.scripts) &&
          this._config.scripts.length > 0;
        if (hasScripts) {
          this._loadExternalScripts(this._config.scripts)
            .then(() => {
  ...
  _calculateEntities() {
    ...
    if (this._config.entities?.length)
      this._config.entities.forEach((entity) => this._entities.add(entity));
    const entityRegex =
